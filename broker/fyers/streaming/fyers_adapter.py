"""
Fyers WebSocket Adapter for Best-Option
...
class FyersAdapter:
    """
    Fyers WebSocket adapter for Best-Option streaming service
    Follows Best-Option adapter pattern similar to Angel, Zerodha etc.
    """
    ...
    def subscribe_symbols(self, symbols: list[dict[str, str]], data_type: str, callback: Callable):
        ...
        try:
            with self.lock:
                ...
                self.logger.debug(
                    f"Converting {len(valid_symbols)} Best-Option symbols to HSM format using database lookup..."
                )

                # Convert Best-Option symbols directly to HSM tokens using database lookup
                hsm_tokens, token_mappings, invalid_symbols = (
                    self.token_converter.convert_Best-Option_symbols_to_hsm(valid_symbols, data_type)
                )
                ...
                # Since convert_Best-Option_symbols_to_hsm processes symbols in order
    ...
    def _on_message(self, fyers_data: dict[str, Any]):
        ...
        try:
            ...
            # Map to Best-Option format first to get symbol info
            ...
            callback = None
            Best-Option_data_type = "Quote"  # Default
            ...
            if Best-Option_data_type == "Depth":
