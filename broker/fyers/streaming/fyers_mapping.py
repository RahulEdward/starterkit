"""
Fyers Data Mapping
Maps Fyers HSM data to Best-Option format for compatibility
"""
...
class FyersDataMapper:
    """
    Maps Fyers HSM WebSocket data to Best-Option format
    """
    ...
    def map_to_Best-Option_ltp(self, fyers_data: dict[str, Any]) -> dict[str, Any] | None:
        """
        Map Fyers data to Best-Option LTP format

        Args:
            fyers_data: Raw data from Fyers HSM WebSocket

        Returns:
            Best-Option LTP format dict or None if mapping fails
        """
        try:
            ...
            return Best-Option_data

        except Exception as e:
            logger.debug(f"Error mapping LTP data: {e}")
            return None

    def map_to_Best-Option_quote(self, fyers_data: dict[str, Any]) -> dict[str, Any] | None:
        """
        Map Fyers data to Best-Option Quote format

        Args:
            fyers_data: Raw data from Fyers HSM WebSocket

        Returns:
            Best-Option Quote format dict or None if mapping fails
        """
        try:
            if not fyers_data:
                return None
            ...
            # Map to Best-Option Quote format
            Best-Option_data = {
                "symbol": f"{exchange}:{symbol_name}",
                "exchange": exchange,
                "token": fyers_data.get("exchange_token", ""),
    ...
    def map_to_Best-Option_depth(self, fyers_data: dict[str, Any]) -> dict[str, Any] | None:
        """
        Map Fyers depth data to Best-Option Depth format

        Args:
            fyers_data: Raw depth data from Fyers HSM WebSocket

        Returns:
            Best-Option Depth format dict or None if mapping fails
        """
        try:
            ...
            # Map to Best-Option Depth format (matching other brokers)
            Best-Option_data = {
                "symbol": f"{exchange}:{symbol_name}",
                "exchange": exchange,
                "token": fyers_data.get("exchange_token", ""),
    ...
    def map_tbt_depth_to_Best-Option(
        self, ticker: str, tbt_depth: dict[str, Any], symbol: str, exchange: str
    ) -> dict[str, Any] | None:
        """
        Map Fyers TBT 50-level depth data to Best-Option Depth format

        Args:
            ticker: Original Fyers ticker (e.g., 'NSE:RELIANCE-EQ')
            tbt_depth: Depth data from TBT WebSocket
            symbol: Best-Option symbol name
            exchange: Best-Option exchange name
        ...
        try:
            ...
            # Map to Best-Option Depth format
            Best-Option_data = {
                "symbol": symbol,
                "exchange": exchange,
                "token": ticker,
    ...
    def map_index_to_synthetic_depth(self, fyers_data: dict[str, Any]) -> dict[str, Any] | None:
        """
        Map Fyers index data to synthetic Best-Option Depth format
    ...
    def map_fyers_data(
        ...
        """
        Map Fyers data to appropriate Best-Option format based on requested type
        ...
        Returns:
            Mapped Best-Option data or None if mapping fails
        """
        if not fyers_data:
            return None
        ...
        if requested_type == "LTP":
            return self.map_to_Best-Option_ltp(fyers_data)
        elif requested_type == "Quote":
            return self.map_to_Best-Option_quote(fyers_data)
        elif requested_type == "Depth" and fyers_type == "dp":
            return self.map_to_Best-Option_depth(fyers_data)
    ...
    def extract_symbol_info(self, symbol: str) -> dict[str, str]:
        """
        Extract exchange and symbol from Best-Option format
