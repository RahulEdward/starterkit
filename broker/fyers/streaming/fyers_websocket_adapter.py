"""
Fyers WebSocket Adapter for Best-Option WebSocket Proxy
Integrates with the Best-Option WebSocket proxy system
"""
...
class FyersWebSocketAdapter(BaseBrokerWebSocketAdapter):
    """Fyers-specific implementation of the WebSocket adapter for Best-Option proxy"""
    ...
    def __init__(self):
        ...
        self.tbt_symbol_to_ticker = {}  # Best-Option symbol -> Fyers ticker
        self.tbt_ticker_to_symbol = {}  # Fyers ticker -> Best-Option symbol
    ...
    def _subscribe_tbt_depth(
        ...
        Args:
            symbol: Best-Option symbol (actual symbol without suffix)
            exchange: Exchange name
    ...
    def _unsubscribe_tbt_depth(self, symbol: str, exchange: str) -> bool:
        ...
        Args:
            symbol: Best-Option symbol (may include :50 suffix)
            exchange: Exchange name
    ...
    def _convert_to_fyers_ticker(self, symbol: str, exchange: str) -> str | None:
        """
        Convert Best-Option symbol to Fyers ticker format using database lookup

        Args:
            symbol: Best-Option symbol (e.g., 'RELIANCE', 'NIFTY24DEC25000CE')
    ...
    def _on_tbt_depth_update(self, ticker: str, depth_data: dict[str, Any]):
        ...
        try:
            ...
            # Map to Best-Option format
            symbol = subscription["symbol"]
            exchange = subscription["exchange"]

            self.logger.debug(f"Mapping TBT depth for {exchange}:{symbol}")

            mapped_data = self.data_mapper.map_tbt_depth_to_Best-Option(
    ...
    def _map_fyers_to_Best-Option(
        self, fyers_data: dict[str, Any], mode: int
    ) -> dict[str, Any] | None:
        """
        Map Fyers data to Best-Option WebSocket format
        ...
        Returns:
            Mapped data in Best-Option format
        """
        try:
            if not fyers_data:
                return None
            ...
            if mode == 1:  # LTP
                ...
                Best-Option_data.update({"ltp": converted_ltp, "data_type": "LTP"})
            elif mode == 2:  # Quote
