# Mapping Best-Option API Request
# Adapted from OpenAlgo (https://openalgo.in/docs)
...
def transform_margin_positions(positions):
    """
    Transform Best-Option margin position format to Angel Broking margin format.

    Args:
        positions: List of positions in Best-Option format
...
def map_product_type(product):
    """
    Maps Best-Option product type to Angel Broking product type.

    Best-Option: CNC, NRML, MIS
...
def map_order_type(pricetype):
    """
    Maps Best-Option price type to Angel Broking order type.

    Best-Option: MARKET, LIMIT, SL, SL-M
...
def parse_margin_response(response_data):
    """
    Parse Angel Broking margin calculator response to Best-Option standard format.

    Args:
        response_data: Raw response from Angel Broking margin calculator API

    Returns:
        Standardized margin response matching Best-Option format
    """
    try:
        ...
        # Return standardized format matching Best-Option API specification
        return {
            "status": "success",
            "data": {
