...
def get_api_response(endpoint, auth, method="GET", payload="", max_retries=2, api_key=None):
    AUTH_TOKEN = auth
...
def get_open_position(tradingsymbol, exchange, producttype, auth):
    # Convert Trading Symbol from Best-Option Format to Broker Format Before Search in OpenPosition
...
def close_all_positions(current_api_key, auth):
    ...
    if positions_response["status"]:
        ...
        for position in positions_response["data"]:
            ...
            # get Best-Option symbol to send to placeorder function
