...
def authenticate_broker(clientcode, broker_pin, totp_code, api_key=None):
    """
    Authenticate with the broker and return the auth token.
    
    Args:
        clientcode: Broker client ID
        broker_pin: Broker PIN/password
        totp_code: 6-digit TOTP code
        api_key: Broker API key (from database, not .env)
    """
    # Use provided api_key or fallback to env (for backward compatibility)
    if not api_key:
        api_key = os.getenv("BROKER_API_KEY")
    
    if not api_key:
        return None, None, "Broker API key not configured"
